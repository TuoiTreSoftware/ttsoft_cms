...
class Rev_addon_Admin {
    ...
    public function update_plugin() {
		...
		if(isset($_REQUEST['plugin'])){
			...
			$plugin_result = false;
			$plugin_message = 'UNKNOWN';
			$url = 'updates.themepunch.tools/revslider-js-addon/addons/'.$_REQUEST['plugin'].'/'.$_REQUEST['plugin'].'.zip';
			...
			if( !$get ){
			  $plugin_message = 'FAILED TO DOWNLOAD';
				...
				$plugin_message = 'ZIP is there';
				$upload_dir = wp_upload_dir();
				$file = $upload_dir['basedir']. '/templates/' . $_REQUEST['plugin'] . '.zip';
                wp_mkdir_p(dirname($file));
				$ret = @file_put_contents( $file, $get['body'] );
				...
				@unlink($file);
				return __('Addon updated.');
			}
		}
		else{
			return __('Failed to update addon.');
		}
	} 
