...
class RevSliderOperations extends RevSliderElementsBase{
	 ...
	 */
	public function previewOutputMarkup($sliderID, $options = null){

	    $_defaultOptions = array(
            'zip_download' => true, //download or save export zip
            'zip_filename' => '', //zip filename in case of save
	        'include_json' => true, //include json in resulting zip
	        'plugin_folder_url' => '',//prefix for css / js / fonts
	        'plugin_folder_assets_url' => 'assets',// prefix for media files only
        );

	    if ( is_null($options) || !is_array($options) ) {
	        $options = array();
        }
        foreach ( array_keys($_defaultOptions) as $key ) {
            if ( !isset($options[$key]) ) {
                $options[$key] = $_defaultOptions[$key];
            }
        }

        if ( !empty($options['plugin_folder_url']) )
            $options['plugin_folder_url'] = rtrim($options['plugin_folder_url'], '/').'/';

        $options['plugin_folder_assets_url'] = rtrim($options['plugin_folder_assets_url'], '/');
		...
		if($export_real){ //set all different file path's here
			$path_fonts = $options['plugin_folder_url'].'fonts/';
			$path_css = $options['plugin_folder_url'].'css/';
			$path_js = $options['plugin_folder_url'].'js/';
			$path_assets = $options['plugin_folder_assets_url'];
			$path_assets_raw = $_defaultOptions['plugin_folder_assets_url'];
			$path_assets_vid = $options['plugin_folder_assets_url'];
			$path_assets_raw_vid = $_defaultOptions['plugin_folder_assets_url'];
		}else{
	...
	<head>
        ...
        <?php
        $stylesString = '';
        $stylesString .= '        <!-- LOADING FONTS AND ICONS -->' . "\n";
        $stylesString .= '        ' . $fonts . "\n";
        $stylesString .= "\n";
        $stylesString .= '		<link rel="stylesheet" type="text/css" href="' . $path_fonts . 'pe-icon-7-stroke/css/pe-icon-7-stroke.css">' . "\n";
        $stylesString .= '		<link rel="stylesheet" type="text/css" href="' . $path_fonts . 'font-awesome/css/font-awesome.css">' . "\n";
        $stylesString .= "\n";
        $stylesString .= '		<!-- REVOLUTION STYLE SHEETS -->' . "\n";
        $stylesString .= '		<link rel="stylesheet" type="text/css" href="' . $path_css.  'settings.css">' . "\n";
        $stylesString .= '		<!-- REVOLUTION LAYERS STYLES -->' . "\n";

		if($export_real){
			$stylesString .= $styles;
			if($static_css !== ''){
				$stylesString .= '<style type="text/css">';
				$stylesString .= RevSliderCssParser::compress_css($static_css);
				$stylesString .= '</style>'."\n";
			}
		}else{
			?>
        ...
        <?php if ($slider->getParam('bubblemorph_enabled', false) == 'true') : ?><link rel="stylesheet" type="text/css" href="<?php echo $path_css; ?>revolution.addon.bubblemorph.css"><?php endif; ?>
        ...
        <?php if ($slider->getParam('liquideffect_enabled', false) == 'true') : ?><link rel="stylesheet" type="text/css" href="<?php echo $path_css; ?>liquideffect.css"><?php endif; ?>
        ...
        <?php if ($slider->getParam('bubblemorph_enabled', false) == 'true') : ?><script type="text/javascript" src="<?php echo $path_js; ?>revolution.addon.bubblemorph.min.js"></script><?php endif; ?>
        ...
        <?php if ($slider->getParam('liquideffect_enabled', false) == 'true') : ?>
            <script type="text/javascript" src="<?php echo $path_js; ?>pixi.min.js"></script>
            <script type="text/javascript" src="<?php echo $path_js; ?>revolution.addon.liquideffect.min.js"></script>
        <?php endif; ?>
        ...
        <?php
        $CI = &get_instance();
        $localizeScripts = $CI->data->get('localize_scripts');
        if ($localizeScripts) : ?>
	...
	<?php if ( ! $export_real) : ?>
        ...
        <script type="text/javascript" src="../../assets/warning.js"></script><?php endif; ?>
    </body>
</html>
<?php
...
ob_end_clean();
		...
		$_replace = array(
			'jquery.themepunch.revolution.min.js"></script>'."\n"
			.'		<script type="text/javascript" src="'.$path_js.'jquery.revslider.embed.js"></script>'."\n"
			.'		<script type="text/javascript" src="'.$path_js.'jquery.revslider-'.$slider->getAlias().'.js"></script>'."\n"
			.'		<script type="text/javascript">'."\n"
			.'		    jQuery(function() {'."\n"
		...
		$json_slider_head = str_replace($_find, $_replace, $head);
        $json_slider_content = $content."\n".$scripts;
		...
		$search = array(
            ...
            'url":"',
            'image":"',
            trim(json_encode(rtrim($cont_url, RS_IMAGE_PATH) . $addonPath), '"'),
            trim(json_encode(rtrim($cont_url_no_www, RS_IMAGE_PATH) . $addonPath), '"')
        );
        ...
        foreach($search as $s){
            ...
            if(!empty($_files) && isset($_files[0]) && !empty($_files[0])){
				...
				foreach($_files[0] as $_file){
                    ...
                    if (defined('WHITEBOARD_PLUGIN_URL') && strpos($s, WHITEBOARD_PLUGIN_URL) !== false) {
                        $_file = str_replace(base_url(), '', $_file);
                    } else {
                    ...
                    preg_match('/.*?.(?:woff2)/i', $_file, $fontMatch);
					...
					if($f == false){
						...
						if(strpos($o, 'public/assets/js/') !== false){ //this will be the jsFileLocation script part
                            ...
                            $json_slider_head = str_replace($o, '"'.$path_js.'"', $json_slider_head);
                            $json_slider_content = str_replace($o, '"'.$path_js.'"', $json_slider_content);
						}
                    ...
                    $isJsonFile = strpos($_file, 'url:') === 0;
                    if ($isJsonFile) {
                        $_file = str_replace(array('url:', '\/', $cont_url, '//'), array('', '/', '', ''), $_file);
                    }
                    $isJsonFilePanorama = strpos($_file, 'image:') === 0;
                    if ($isJsonFilePanorama) {
                        $_file = str_replace(array('image:', '\/', $cont_url, '//'), array('', '/', '', ''), $_file);
                    }
                    ...
                    if(is_file($upload_dir.$_file)){
						...
						$remove = true;
                    }elseif(is_file($addonDir.$_file)){
                        $mf = str_replace('//', '/', $addonDir.$_file);
                        if(!$usepcl){
                            $zip->addFile($mf, $use_path_raw.'/'.$repl_to);
                        }else{
                            $v_list = $pclzip->add($mf, PCLZIP_OPT_REMOVE_PATH, str_replace(basename($mf), '', $mf), PCLZIP_OPT_ADD_PATH, $use_path_raw.'/');
                        ...
                        if(defined('WHITEBOARD_PLUGIN_PATH')){
                            if(is_file(FCPATH.$_file)){
                                $mf = str_replace('//', '/', FCPATH.$_file);
					...
					if($remove == true){
						...
						if ($isJsonFile) {
                            $use_path = 'url":"' . $use_path . '\\';
                        }
                        if ($isJsonFilePanorama) {
                            $use_path = 'image":"' . $use_path . '\\';
                        }

						$slider_html = str_replace($o, $reOpen.$use_path.'/'.$repl_to.$reClose, $slider_html);
                        $json_slider_head = str_replace($o, '"'.$use_path.'/'.$repl_to.'"', $json_slider_head);
                        $json_slider_content = str_replace($o, '"'.$use_path.'/'.$repl_to.'"', $json_slider_content);
					}
				}
        ...
        foreach ($dotOverlays as $dotClass => $dotFile) {
			if (strpos($slider_html, 'dottedOverlay:"' . $dotClass . '"') !== false) {
                if( ! $usepcl) {
                    $zip->addFile(RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, $path_assets_raw.'/'.$dotFile);
                }else{
                    $pclzip->add(RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/', $path_assets_raw.'/');
                }
            }
        }

        $json_assets = array();
		preg_match_all('/href="([^"]+?)"/', $head, $matches);
        if (isset($matches[1])) {
            foreach ($matches[1] as $asset) {
                if ( ! in_array($asset, $json_assets)) {
                    $json_assets[] = $asset;
                }
            }
        }
        $json_slider_html = $json_slider_head."\n".'		<div class="revslider" data-alias="' . $slider->getAlias() .  '"></div>'."\n".$footer;
        $json_slider_data = array(
            'locations' => array(
                'fonts'  => $path_fonts,
                'css'    => $path_css,
                'js'     => $path_js,
            ...
            'content'   => $json_slider_content,
			'styles'	=> $stylesString
        );
        $json_slider_data_json = RevSliderFunctions::jsonEncodeForClientSide($json_slider_data);
        $json_slider_js = "jQuery(function(){jQuery().embedRevslider('load', $json_slider_data_json)});";
		...
		if($export_real){ //only include if real export
            ...
            if(!$usepcl){
                ...
                if ($slider->getParam('bubblemorph_enabled', false) == 'true') {
                    $zip->addFile(RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/js/revolution.addon.bubblemorph.min.js', 'js/revolution.addon.bubblemorph.min.js');
                    $zip->addFile(RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/css/revolution.addon.bubblemorph.css', 'css/revolution.addon.bubblemorph.css');
                }
                ...
                if ($slider->getParam('liquideffect_enabled', false) == 'true') {
                    $zip->addFile(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/pixi.min.js', 'js/pixi.min.js');
                    $zip->addFile(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/revolution.addon.liquideffect.min.js', 'js/revolution.addon.liquideffect.min.js');
                    $zip->addFile(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/css/liquideffect.css', 'css/liquideffect.css');
                }
                ...
                if ($slider->getParam('revslider-weather-enabled', false) == 'true') {
                    $zip->addFile(FCPATH . '/plugins/revslider-weather-addon/public/js/revslider-weather-addon-public.js', 'js/revslider-weather-addon-public.js');
                    $zip->addFile(FCPATH . '/plugins/revslider-weather-addon/public/css/revslider-weather-addon-icon.css', 'css/revslider-weather-addon-icon.css');
                    $zip->addFile(FCPATH . '/plugins/revslider-weather-addon/public/css/revslider-weather-addon-public.css', 'css/revslider-weather-addon-public.css');
                }
                ...
                if ( $options['include_json'] ) {
                    $zip->addFile(FCPATH . '/assets/js/jquery.revslider.embed.js', 'js/jquery.revslider.embed.js');
                }
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/css/settings.css', 'css/settings.css');
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/css/closedhand.cur', 'css/closedhand.cur');
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/css/openhand.cur', 'css/openhand.cur');
                ...
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', 'fonts/font-awesome/fonts/fontawesome-webfont.woff2');
                ...
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/assets/coloredbg.png', 'assets/coloredbg.png');
                $zip->addFile(RS_PLUGIN_PATH.'/public/assets/assets/loader.gif', 'assets/loader.gif');
            }else{
                ...
                if ($slider->getParam('bubblemorph_enabled', false) == 'true') {
                    $pclzip->add(RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/js/revolution.addon.bubblemorph.min.js', PCLZIP_OPT_REMOVE_PATH, RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                    $pclzip->add(RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/css/revolution.addon.bubblemorph.css', PCLZIP_OPT_REMOVE_PATH, RS_BUBBLEMORPH_PLUGIN_PATH . 'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                }
                ...
                if ($slider->getParam('liquideffect_enabled', false) == 'true') {
                    $pclzip->add(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/pixi.min.js', PCLZIP_OPT_REMOVE_PATH, RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                    $pclzip->add(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/revolution.addon.liquideffect.min.js', PCLZIP_OPT_REMOVE_PATH, RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                    $pclzip->add(RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/css/liquideffect.css', PCLZIP_OPT_REMOVE_PATH, RS_LIQUIDEFFECT_PLUGIN_PATH . 'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                }
                ...
                if ($slider->getParam('revslider-weather-enabled', false) == 'true') {
                    $pclzip->add(FCPATH . '/plugins/revslider-weather-addon/public/js/revslider-weather-addon-public.js', PCLZIP_OPT_REMOVE_PATH, FCPATH . '/plugins/revslider-weather-addon/public/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                    $pclzip->add(FCPATH . '/plugins/revslider-weather-addon/public/css/revslider-weather-addon-icon.css', PCLZIP_OPT_REMOVE_PATH, FCPATH . '/plugins/revslider-weather-addon/public/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                    $pclzip->add(FCPATH . '/plugins/revslider-weather-addon/public/css/revslider-weather-addon-public.css', PCLZIP_OPT_REMOVE_PATH, FCPATH . '/plugins/revslider-weather-addon/public/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                }
                ...
                if ( $options['include_json'] ) {
                    $pclzip->add(FCPATH . '/assets/js/jquery.revslider.embed.js', PCLZIP_OPT_REMOVE_PATH, FCPATH . 'assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                }
                ...
                $pclzip->add(RS_PLUGIN_PATH.'public/assets/css/closedhand.cur', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                $pclzip->add(RS_PLUGIN_PATH.'public/assets/css/openhand.cur', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
                ...
                $pclzip->add(RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/');
                ...
                $pclzip->add(RS_PLUGIN_PATH.'/public/assets/assets/coloredbg.png', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(RS_PLUGIN_PATH.'/public/assets/assets/loader.gif', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/');
            }
        ...
        $slider_html = apply_filters('revslider_previewOutputMarkup_before_zip', $slider_html, $slider);

        if(!$usepcl){
            $zip->addFromString("slider.html", $slider_html); //add slider settings
            if ( $options['include_json'] ) {
                $zip->addFromString("json_slider.html", $json_slider_html);
                $zip->addFromString("js/jquery.revslider-" . $slider->getAlias() . ".js", $json_slider_js);
            }
            $zip->close();
        }else{
			$pclzip->add(array(array( PCLZIP_ATT_FILE_NAME => 'slider.html',PCLZIP_ATT_FILE_CONTENT => $slider_html)));
            if ( $options['include_json'] ) {
                $pclzip->add(array(array(PCLZIP_ATT_FILE_NAME => 'json_slider.html', PCLZIP_ATT_FILE_CONTENT => $json_slider_html)));
                $pclzip->add(array(array(PCLZIP_ATT_FILE_NAME => 'js/jquery.revslider-' . $slider->getAlias() . '.js', PCLZIP_ATT_FILE_CONTENT => $json_slider_js)));
            }
        }

        if ( !$options['zip_download'] && $options['zip_filename'] ) {
            copy(RevSliderGlobals::$uploadsUrlExportZip, $options['zip_filename']);
            @unlink(RevSliderGlobals::$uploadsUrlExportZip); //delete file after sending it to user
            return $options['zip_filename'];
        } else {
	...
	public function updateGeneralSettings($data){
        ...
        $configString = "<?php\n"
            . "define('RS_CI_DEV_MODE', " . (isset($data['enable_dev_mode']) && $data['enable_dev_mode'] == 'on' ? 'true' : 'false') . ");\n"
            . "define('RS_CI_LOGS', " . (isset($data['logging_level']) ? $data['logging_level'] : 0) . ");\n";
        file_put_contents(APPPATH . 'config/dev.php', $configString);
	}
	...
	public function modifyCustomSliderParams($data){
		...
		$data['main']['alias'] = strtolower(url_title($data['main']['alias']));
...
$presets[] = array (
	...
	/**
	 * get general settigns option value
	 */

	public static function getGeneralSettingsOptionValue($option, $default = false) {
		$arrValues = self::getGeneralSettingsValues();
		return RevSliderBase::getVar($arrValues, $option, $default);
	}
    ...
    public static function create_slider_page($added){
        ...
        foreach($added as $sid){
            ...
            $alias = $slider->getAlias();
            if($alias !== ''){
                $content .= 'RevSliderEmbedder::putRevSlider(\'' . $alias . '\');' . "\n"; //this way we will reorder as last comes first
            }
        }
        ...
        if($content !== '') {
            ...
            $pageDir = 'pages';
            $pagePath = FCPATH . $pageDir . DIRECTORY_SEPARATOR;
            if (wp_mkdir_p($pagePath) && is_writable($pagePath)) {

                $pageFile = 'revslider-' . $page_id . '-' . sanitize_title_with_dashes($alias) . '.php';
                $pageContent = str_replace(
                    array('{title}', '{content}'),
                    array($slider->getTitle(), $content),
                    file_get_contents(APPPATH . 'views/templates/page_template.html')
                );

                if (file_put_contents($pagePath . $pageFile, $pageContent)) {
                    $new_page_id = base_url() . $pageDir . '/' . $pageFile;
                    $page_id++;
