...
class image_CRUD
{
    ...
    protected $priority_direction = 'asc';
    ...
    protected $type_extensions = array(
        'image' => array('gif', 'jpeg', 'jpg', 'png'),
        'video' => array('mp4', 'mp3', 'webm', 'ogv')
    );
    protected $video_thumbnail = 'assets/image_crud/images/icon_video.png';
    ...
    function set_ordering_direction($field_name)
    {
        $this->priority_direction = $field_name;
    ...
    protected function _upload_file($upload_dir, $type = 'image')
    {

        $reg_exp = '/(\\.|\\/)(' . implode('|', $this->type_extensions[$type]) . ')$/i';
    ...
    {
        $rsegments_array = $this->_rsegment_array();
        return $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/delete_file/' . $value);
    }

    protected function _get_photos($relation_value = null, $type = 'image')
    {
        if (!empty($this->priority_field)) {
            $this->ci->db->order_by($this->priority_field, $this->priority_direction);
        }
        ...
        foreach ($results as $num => $row) {
            $ext = strtolower(pathinfo($this->image_path . '/' . $row->{$this->url_field}, PATHINFO_EXTENSION));
            if (!in_array($ext, $this->type_extensions[$type])) {
                unset($results[$num]);
                continue;
            }

            if ($type == 'image') {
                $_image_url = str_replace(array('/', '\\'), '-', $row->{$this->url_field});
                if (!file_exists($this->image_path . '/' . $this->thumbnail_prefix . $_image_url)) {
                    $this->_create_thumbnail($this->image_path . '/' . $row->{$this->url_field}, $this->image_path . '/' . $this->thumbnail_prefix . $_image_url);
                }
            }

            $results[$num]->image_url = base_url() . $this->image_path . '/' . $row->{$this->url_field};
            if ($type == 'image') {
                $results[$num]->thumbnail_url = base_url() . $this->image_path . '/' . $this->thumbnail_prefix . $_image_url;
            } else {
                $results[$num]->thumbnail_url = base_url() . $this->video_thumbnail;
            }
            $results[$num]->delete_url = $this->_get_delete_url($row->{$this->primary_key});
            $results[$num]->type = $type;
            $results[$num]->file_name = $row->{$this->url_field};
        }
    ...
    {
        $sizes = $this->ci->config->item('rs_image_sizes');
        $size = $sizes['gallery'];

        $this->image_moo
            ->load($image_path)
            ->resize_crop($size['width'], $size['height'])
    ...
    protected function getState()
    {
        ...
        $type = $this->ci->input->get('t') ? $this->ci->input->get('t') : 'image';

        if (isset($rsegments_array[3]) && is_numeric($rsegments_array[3])) {
            $upload_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/upload_file/' . $rsegments_array[3]);
            $ajax_list_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/' . $rsegments_array[3] . '/ajax_list');
            $ordering_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/ordering');
            $insert_title_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/insert_title');
            ...
            $upload_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/upload_file');
            $ajax_list_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/ajax_list');
            $ordering_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/ordering');
            $insert_title_url = $this->_site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/insert_title');
        ...
        if (!empty($state_info)) {
            if (RS_DEMO) {
                switch ($state_info->name) {
                    case 'upload_file':
                    case 'delete_file':
                        throw new Exception('Not available in Demo Mode', 1);
                        die();
                        break;
                }
            }
            ...
            switch ($state_info->name) {
                case 'list':
                    $photos = isset($state_info->relation_value) ? $this->_get_photos($state_info->relation_value) : $this->_get_photos(null, $state_info->type);
                ...
                case 'upload_file':
                    ...
                    $uploadResult = $this->_upload_file($this->image_path, $state_info->type);
                    ...
                    if ($uploadResult['success'] && $uploadResult['filename']) {
                        ...
                        $this->_insert_table($file_name, isset($state_info->relation_value) ? $state_info->relation_value : '');
                    }

                    @ob_end_clean();
                    echo json_encode((object) $uploadResult);
    ...
    /**
     *    Fix for no mod rewite routing
     */

    protected function _rsegment_array()
    {
        $segments = array(
            '',
            $this->ci->input->get('c') ? $this->ci->input->get('c') : 'media',
            $this->ci->input->get('m') ? $this->ci->input->get('m') : 'index',
            $this->ci->input->get('f'),
        );
        if ($this->ci->input->get('a')) {
            $segments[] = $this->ci->input->get('a');
        }
        return $segments;
    }

    protected function _site_url($url)
    {
        $keys = array('c', 'm', 'f', 'a');
        foreach (explode('/', $url) as $key => $value) {
            $arrUrl[] = $keys[$key] . '=' . $value;
        }
        return site_url(implode('&', $arrUrl));
    }
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error)
    {
        ...
        if (!$error && $file->name) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    $uploadResult = @file_put_contents(
                        $file_path,
                        fopen($uploaded_file, 'r'),
                        FILE_APPEND
                    );
                } else {
                    $uploadResult = @move_uploaded_file($uploaded_file, $file_path);
                }
            } else {
            ...
            if ($uploadResult === false) {
                $file->error = "Failed to write uploaded file to '$file_path'";
            } else {
