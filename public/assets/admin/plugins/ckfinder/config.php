...
function CheckAuthentication()
{
	...
	//return (Auth::guard()->check()) ? true : false;
	return true;

}

// LicenseKey : Paste yoreturn (Auth::guard()->check()) ? true : false;ur license key here. If left blank, CKFinder will be
...
$config['AccessControl'][] = Array(
		...
		// 'folderView' 	=> (Auth::guard()->user()->can('folderView')) ? true : false,
		// 'folderCreate' 	=> (Auth::guard()->user()->can('folderCreate')) ? true : false,
		// 'folderRename' 	=> (Auth::guard()->user()->can('folderRename')) ? true : false,
		// 'folderDelete' 	=> (Auth::guard()->user()->can('folderDelete')) ? true : false,

		// 'fileView' 		=> (Auth::guard()->user()->can('fileView')) ? true : false,
		// 'fileUpload' 	=> (Auth::guard()->user()->can('fileUpload')) ? true : false,
		// 'fileRename' 	=> (Auth::guard()->user()->can('fileRename')) ? true : false,
		// 'fileDelete' 	=> (Auth::guard()->user()->can('fileDelete')) ? true : false
...
$config['plugin_imageresize']['smallThumb'] = '210x144';
$config['plugin_imageresize']['mediumThumb'] = '350x263';
$config['plugin_imageresize']['largeThumb'] = '456x342';
