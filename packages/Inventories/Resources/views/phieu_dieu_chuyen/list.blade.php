	...
	<div class="row page-titles">
		...
		<div class="col-md-7 align-self-center text-right">
			<div class="d-flex justify-content-end align-items-center">
				<a href="{{ route('inventories.dieu_chuyen.get.add') }}" class="btn btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Tạo Phiếu Nhập Kho</a>
			</div>
		</div>
	</div>
	<div class="row">
						...
						<table class="table table-bordered table-hover product-overview">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ get_customer_name_by_id($val->customer_id) }}</td>
									<td>{{ $val->user_id }}</td>
									<td class="text-right"><a href="#">{{ getTotalDocHeadersById($val->getId()) }}</a></td>
									<td>{{ $val->description }}</td>
									<td>{{ get_category_name_by_id($val->status_id) }}</td>
									<td><a href="#" class="text-dark p-r-10" data-toggle="tooltip" title="Edit"><i class="ti-marker-alt"></i></a> <a href="{{ route('inventories.nhap_kho.get.delete',$val->getId()) }}" class="text-dark" title="Delete" data-toggle="tooltip"><i class="ti-trash"></i></a></td>
								</tr>
