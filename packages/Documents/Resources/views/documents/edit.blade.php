	...
	<div class="row">
		<div class="col-lg-12">
			<div class="card">
				...
				<div class="card-body">
					<form action="{{ route('post.edit.document',$document->id) }}" method="post">
						{{ csrf_field() }}
						<div class="row">
							...
							<div class="col-md-4">
								...
								<select name="version_id" class="select2">
									...
									<option value="{{ $version->id }}" @if($document->version_id == $version->id) selected @endif>{{ $version->title }}</option>
									@endforeach
								</select>
							</div>
							<div class="col-md-4">
								...
								<select name="data_href_menu" class="select2">
									...
									<option value="{{ $menu->data_href }}" @if($document->data_href_menu == $menu->data_href) selected @endif>{{ $menu->title }}</option>
									@endforeach
								</select>
							</div>
							...
							<div class="form-group col-md-12">
								...
								{!! ckeditorContent($nameFile = 'content',$old = $document->content, $folder = 'documents', $idContent = 'content' ,$title = trans('Nội dung'),$height = 400) !!}
							</div>
						</div>
