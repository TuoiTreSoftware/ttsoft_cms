	...
	<div class="row">
		<div class="col-lg-4">
			<div class="card">
				...
				<div class="card-body">
					<form action="{{ route('post.edit.document_version',$version->id) }}" method="post">
						{{ csrf_field() }}
						<div class="form-group">
							<label for="title" class="font-weight-bold">Version</label>
							<input class="form-control" name="title" value="{{ $version->title }}">
						</div>
						<div class="form-group">
							<label for="title" class="font-weight-bold">Description</label>
							<textarea name="description" class="form-control">{{ $version->description }}</textarea>
						</div>
