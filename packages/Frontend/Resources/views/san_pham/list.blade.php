   ...
   <div class="container clearfix">
      ...
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item active" aria-current="page">
        <a href={{ $products_title->category->getRoute() }}>
          {{ $products_title->category->getTitle() }}
        </a>
      </li>
      </ol>
   </div>
                      ...
                      <div class="flexslider">
                        <div class="slider-wrap" data-lightbox="gallery">
                          @foreach($product->all_image as $key => $c)
                            <div class="slide"><a href="{{ $c }}" data-lightbox="gallery-item"><img src="{{ $c }}" alt="{{ $product->getTitle() }}" ></a></div>
                          @endforeach
                        </div>
                      </div>
                    </div>
                    {{-- <div class="product-cart"><a href="#"><i class="icon-shopping-cart"></i></a></div> --}}
                    <div class="product-quickview"><a href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="Sizes: @foreach($product->getSizeAttribute() as $key => $val) {{$val->title}} @endforeach"><i class="icon-info"></i></a>
                    </div>
                  </div>
