...
<div class="container-fluid">
	<div class="row page-titles">
		<div class="col-md-5 align-self-center">
			<h4 class="text-themecolor">Cập nhật phiếu thu</h4>
		</div>
		<div class="col-md-7 align-self-center text-right">
			<div class="d-flex justify-content-end align-items-center">
				<a href="{{ route('accounting.bao_co.get.list') }}" class="btn btn-info d-none d-lg-block m-l-15"><i class="fa fa-arrow-left"></i> Danh sách phiếu thu</a>
			</div>
		</div>
	</div>
	<div class="row">
				...
				<div class="card-body">
					<form method="post" action="" id="my-form">
						...
						<div class="row">
							...
							<div class="form-group col-md-4">
								<label for="inputbank_accounts" class=" control-label col-form-label">Chọn tài khoảng</label>
								<select class="form-control" id="inputbank_accounts" name="bank_accounts">
									<option value="0">Chọn tài khoảng</option>
									{{ get_category_by_prefix('BANK','',0,$data->bank_accounts) }}
								</select>
							</div>
							<div class="form-group col-md-4">
									...
									<table class="table table-hover table-bordered">
										...
										<tbody>
											@if($data->doc_details()->count() > 0)
												@foreach($data->doc_details as $k => $value)
													<tr>
														...
														<td>
															<input class="form-control change-giatrivat" type="number" name="doc_details[{{$k}}][gia_tri_vat]" id="gia_tri_vat{{$k}}" value="{{ $value->gia_tri_vat }}" data-value="{{ $k }}">
														</td>
														<td>
														...
														<td>
															<input class="form-control thanhtien" readonly="" type="number" name="doc_details[{{ $k }}][thanh_tien]" id="thanh_tien{{$k}}" value="{{ $value->gia_tri + $value->gia_tri_vat }}">
														</td>
														<td>
															<input class="form-control" type="text" name="doc_details[{{$k}}][khoang_muc_phi]" id="khoang_muc_phi[{{$k}}]" value="{{ $value->khoang_muc_phi }}">
														</td>
														<td>
															<button type="button" href="javascript:void(0)" id="" class="btn btn-sm btn-danger btn_clear">Xóa</button>
														</td>
													</tr>
												@endforeach
											@endif
											...
											<tr>
												...
												<td colspan="2"><input type="" name="total" style="color: red;" readonly="" id="total" class="form-control total" value="{{ getTotalDocHeadersById($data->getId()) }}">
												</td>
...
<script  type="text/javascript" >
	...
	$(document).ready(function() {
		...
		$("#btn_add_row").click(function() {
			...
			html += '<tr>';
			html += '<td>'+x+'</td>';
			html += '<td><input class="form-control change-giatri" type="number" name="doc_details['+x+'][gia_tri]" id="gia_tri'+x+'" data-value="'+x+'"></td>';
			html += '<td><input class="form-control change-giatrivat" type="number" name="doc_details['+x+'][gia_tri_vat]" id="gia_tri_vat'+x+'" data-value="'+x+'"s></td>';
		...
		//Tính thành tiền khi nhập giá trị đã thêm
			...
			$("body").on('change', '.change-giatrivat', function() {
				var x = $(this).attr('data-value');
				thanh_tien_x(x);
			});
